...
async function dailyPnl(creds: Awaited<ReturnType<typeof getCredsForUser>>, openPositions: any[]) {
  ...
  return { realized, unrealized, total: realized + unrealized, consecutiveLosses };
}
...
export async function runLocalBotTick(userId: string) {
  ...
  const pnl = await dailyPnl(creds, activeOpenPositions);
  const dailyTarget =
  ...
  const btcOrderSize = paperHighRisk
    ...
    : Math.max(BTC_MIN_ORDER_USDT, effectiveCapital / maxOpenTrades);
  const btcCfg = state.symbols.find((s) => s.symbol === BTC_SYMBOL);
  updateLocalSymbol(userId, BTC_SYMBOL, {
    enabled: true,
    grid_levels: paperHighRisk ? 3 : BTC_GRID_LEVELS,
    order_size_usdt: btcOrderSize,
    min_order_size_usdt: BTC_MIN_ORDER_USDT,
    max_order_size_usdt: Math.max(BTC_MIN_ORDER_USDT, btcOrderSize * 2),
    grid_spacing_pct: paperHighRisk ? 0.25 : BTC_FAST_SPACING_PCT,
    leverage: paperHighRisk ? 8 : BTC_LEVERAGE,
    stop_loss_roi_pct: Number(btcCfg?.stop_loss_roi_pct ?? (paperHighRisk ? -8 : -12)),
    ...
    funding_max_abs_bps: Number(btcCfg?.funding_max_abs_bps ?? 10),
    z_filter_enabled: Boolean(btcCfg?.z_filter_enabled ?? false),
    z_entry_threshold: Number(btcCfg?.z_entry_threshold ?? (paperHighRisk ? 1.25 : 1.0)),
  });
  ...
  const entriesBlocked = activeOpenPositions.length >= maxOpenTrades || effectiveCapital <= 0;
